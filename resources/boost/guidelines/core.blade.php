...
- `User` carries `is_platform_admin` (a cross-tenant platform operator) and a nullable `account_id` (a billing account owned by `misaf/vendra-subscription`, exposed via `isAccountOwner()`). `canAccessPanel()` gates the app's panels by these: `platform` → `is_platform_admin`, `account` → `isAccountOwner()`, plus the role-based `admin`/`user` arms. Keep `account_id` a plain nullable column with no relation here (no dependency on the subscription module).
