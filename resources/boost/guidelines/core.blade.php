...
- Tenant awareness is owned by `misaf/vendra-support` via the bound `TenantResolver`; consume it through `Misaf\VendraSupport\Tenancy\TenantAwareness` and `BelongsToTenant`, not a `tenant_aware` config toggle.
...
- `User` is the single canonical identity for tenant and platform users alike: a null `tenant_id` marks a platform-level identity (console user, reseller user). Platform guards (`console`, `reseller`) resolve it through their own providers, `console` and `reseller`, both on the `platform-eloquent` driver (`Auth\PlatformUserProvider`, `tenant_id IS NULL` on every lookup); the `web` guard keeps the plain `users` provider. Reset tokens are keyed by email alone, so every scope keeps its own token store: `console` resets write `console_password_reset_tokens`, `reseller` resets write `reseller_password_reset_tokens`, `users` resets write `password_reset_tokens`. Non-admin panel access is delegated to `Support\PanelAccessRegistry` resolvers registered by the owning packages — never add console/reseller table knowledge here.
- Tenant administrator membership changes go through `AddTenantAdministratorAction`, `PromoteTenantAdministratorAction`, `DemoteTenantAdministratorAction`, `RemoveTenantAdministratorAction`, and `SetUserAccountEnabledAction`. Keep the tenant lock and final-enabled-administrator invariant centralized in `Support\TenantAdministratorGuard`; credential changes use `UpdateUserEmailAction` and `UpdateUserPasswordAction`.
