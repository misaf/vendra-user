...
- Two-factor authentication is Filament's authenticator-app provider: `User` implements `HasAppAuthentication` and `HasAppAuthenticationRecovery` through Filament's `InteractsWithAppAuthentication*` traits, which cast `app_authentication_secret` and `app_authentication_recovery_codes` encrypted and hide them. A panel opts in with `->multiFactorAuthentication(AppAuthentication::make()->recoverable())`; the admin panel registers no provider and never challenges. Removing a lost authenticator goes through `ResetUserAppAuthenticationAction`, never a hand-written column update; tests create a user who already has one with `UserFactory::withAppAuthentication()`.
- Validate credentials in forms and commands through `Support\UserRules` (`username()`, `email()`, `password()`, `unique($column, $tenantId, $ignoreUserId)`, `exists($column, $tenantId)`). Username length constants drive form inputs; `password()` delegates strength to the application `Password::default()` policy, and `Support\PasswordGenerator::generate()` reads that same policy so a generated credential always passes it — never hand-roll `Str::password()` for a user password. Keep required/optional, confirmation, and scoped uniqueness at the caller, never a hand-built `Rule::unique(User::class, …)` or `Rule::exists(User::class, …)`. Soft-deleted users release their values, matching the `users_active_username_unique` and `users_active_email_unique` indexes.
