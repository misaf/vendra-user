...
- Validate credentials in forms and commands through `Support\UserRules` (`username()`, `email()`, `password()`, `unique($column, $tenantId, $ignoreUserId)`). Username length constants drive form inputs; `password()` delegates strength to the application `Password::default()` policy, and `generatePassword()` reads that same policy so a generated credential always passes it — never hand-roll `Str::password()` for a user password. Keep required/optional, confirmation, and scoped uniqueness at the caller, never a hand-built `Rule::unique(User::class, …)`. Soft-deleted users release their values, matching the `users_active_username_unique` and `users_active_email_unique` indexes.
